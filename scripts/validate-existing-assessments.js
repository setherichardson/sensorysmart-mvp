// Script to validate existing assessment results in the database
// This will help identify any assessments that may have incorrect profiles
...
async function validateExistingAssessments() {
  console.log("🔍 Validating existing assessment results...\n");
  ...
  try {
    ...
    console.log(`Found ${assessments.length} assessments to validate.\n`);

    let correctProfiles = 0;
    let incorrectProfiles = 0;
    const issues = [];
    ...
    for (const assessment of assessments) {
      ...
      if (!responses || !originalProfile) {
        console.log(`⚠️  Assessment ${assessment.id}: Missing data`);
        continue;
      }
      ...
      if (recalculatedProfile.profile === originalProfile) {
        correctProfiles++;
        console.log(`✅ ${childName}: ${originalProfile} (correct)`);
      } else {
        incorrectProfiles++;
        issues.push({
          id: assessment.id,
          childName,
          originalProfile,
          recalculatedProfile: recalculatedProfile.profile,
          behaviorScores: recalculatedProfile.behaviorScores,
          completedAt: assessment.completed_at
        });
        console.log(`❌ ${childName}: ${originalProfile} → ${recalculatedProfile.profile} (INCORRECT)`);
        console.log(`   Behavior scores:`, recalculatedProfile.behaviorScores);
      }
    }

    console.log(`\n📊 Validation Results:`);
    console.log(`✅ Correct profiles: ${correctProfiles}`);
    console.log(`❌ Incorrect profiles: ${incorrectProfiles}`);
    console.log(`📈 Accuracy: ${((correctProfiles / (correctProfiles + incorrectProfiles)) * 100).toFixed(1)}%`);

    if (issues.length > 0) {
      console.log(`\n🚨 Issues Found:`);
      issues.forEach(issue => {
        console.log(`\n   Assessment ID: ${issue.id}`);
        console.log(`   Child: ${issue.childName}`);
        console.log(`   Original: ${issue.originalProfile}`);
        console.log(`   Recalculated: ${issue.recalculatedProfile}`);
        console.log(`   Behavior Scores:`, issue.behaviorScores);
        console.log(`   Completed: ${new Date(issue.completedAt).toLocaleDateString()}`);
      });

      console.log(`\n💡 Recommendation: Consider updating the incorrect assessments in the database.`);
    } else {
      console.log(`\n🎉 All assessments have correct profiles!`);
    }
